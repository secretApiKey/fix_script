...
wait_for_dns_delegation() {
    local resolver answer attempt max_attempts sleep_seconds

    [ "$GENERATE_CLOUDFLARE_RECORDS" = "1" ] || return 0
    command -v dig >/dev/null 2>&1 || return 0

    max_attempts="${DNS_DELEGATION_WAIT_ATTEMPTS:-30}"
    sleep_seconds="${DNS_DELEGATION_WAIT_SECONDS:-10}"

    echo "Waiting for public DNS delegation: ${NAMESERVER} -> ${DOMAIN}"
    for attempt in $(seq 1 "$max_attempts"); do
        for resolver in 1.1.1.1 8.8.8.8; do
            answer="$(dig +short NS "$NAMESERVER" "@${resolver}" 2>/dev/null | sed 's/\.$//' | head -n 1 || true)"
            if [ "$answer" = "$DOMAIN" ]; then
                echo "DNS delegation visible through ${resolver}: ${NAMESERVER} -> ${DOMAIN}"
                return 0
            fi
        done
        echo "DNS delegation not visible yet (${attempt}/${max_attempts}); waiting ${sleep_seconds}s..."
        sleep "$sleep_seconds"
    done

    echo "WARNING: Cloudflare delegation was created, but public resolvers may still cache NXDOMAIN for ${NAMESERVER}."
    echo "DNSTT may fail until resolver cache expires."
}
