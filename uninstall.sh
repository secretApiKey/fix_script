...
if [ "$SKIP_CONFIRM" != "1" ]; then
    echo "This will remove the installed VIPER PANEL stack from this VPS."
...
for service in xray ErwanDNSTT ErwanDNS ErwanWS ErwanTCP ErwanTLS udp udp2 badvpn-udpgw ddos stunnel4 squid nginx openvpn-server@tcp openvpn-server@udp erwanssh erwan-openvpn-nat erwan-dns-forwarding; do
...
rm -f /etc/systemd/system/udp2.service
rm -f /etc/systemd/system/ddos.service
rm -f /etc/systemd/system/erwanssh.service
rm -f /etc/systemd/system/erwan-openvpn-nat.service
rm -f /etc/systemd/system/erwan-dns-forwarding.service
...
rm -f /etc/cron.d/user-expiry
rm -f /etc/cron.d/account-expiry
rm -f /etc/cron.d/xray-limit
rm -f /etc/cron.d/useradd-limit
rm -f /etc/cron.d/erwan-restart
rm -f /etc/profile.d/juan.sh
rm -f /etc/profile.d/erwan.sh
rm -f /usr/bin/menu
rm -f /usr/bin/extenduser
rm -f /usr/bin/checkuser
rm -f /usr/bin/activelogins
rm -f /usr/bin/xray-menu
rm -f /usr/local/bin/erwan-openvpn-nat
rm -f /usr/local/bin/erwan-dns-forwarding
rm -f /usr/libexec/sshd-auth
rm -f /etc/banner
rm -f /etc/issue.net
...
echo "VIPER PANEL has been removed."
echo "Packages were left installed to avoid damaging the VPS base system."
echo "For a full purge, run uninstall-hard.sh before normal uninstall removes ${TARGET_DIR}."
